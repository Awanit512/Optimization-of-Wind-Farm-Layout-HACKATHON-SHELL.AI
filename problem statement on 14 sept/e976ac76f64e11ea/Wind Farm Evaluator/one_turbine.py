...
def placed_one_turbines(rows,columns,turb_rad, turb_coords, power_curve, wind_inst_freq,d,dc):

	p_x=0.0
	p_y=0.0
	power=0.0
	for rr in np.arange(50,3951,d):
		for cc in np.arange(50,3951,d):
...
def save(file_name,turb_coords):
    ...
    with open(file_name,'w') as csv_file:
        ...
        for ii in turb_coords:
            ...
            d.append(ii[1])
            csv_writer.writerow(d)
    print('file saved succesfully with one coordinates updated')
...
if __name__ == "__main__":
    ...
    d=int(input('min distance between the turbines:'))
	...
	turb_coords=placed_two_turbines(79,79,turb_rad, turb_coords, power_curve, wind_inst_freq,d,dc)
	for i in turb_coords:
		print(i)
