...
def construct_G(sx,sy,ex,ey):
    ...
    for i in np.arange(sx+0.0,ex+1.0,10.0):
        col_r=[]
        col_c=[]
        for j in np.arange(sy+0.0,ey+1.0,10.0):
            col_r.append(i)
            col_c.append(j)
            if flag==0:
                column+=1
...
def greedy_F(k,offset,G_r_x,G_c_y,rs_start,rows,cs_start,columns,S,r_x,r_y,turb_rad,power_curve,XI_x,XI_y,dc):
    ...
    while i<k+offset:
        ...
        flag=0
        max_power=0.000
        n_wind_instances,cos_dir,sin_dir,wind_sped_stacked,C_t = preProcessing(power_curve,42+i+1)
...
if __name__ == "__main__":
    ...
    # S[][]=1
    # 
    print("JSUT A AN INTRO PICTURE BAKI H ABHI MERRE DOST")
    t=1
    for i,j in zip(XI_x,XI_y):
    ...
    XI_x,XI_y = greedy_F(8,0,G_r_x,G_c_y,40-40,391,40-40,391,S,r_x,r_y,turb_rad,power_curve,XI_x,XI_y,dc)
