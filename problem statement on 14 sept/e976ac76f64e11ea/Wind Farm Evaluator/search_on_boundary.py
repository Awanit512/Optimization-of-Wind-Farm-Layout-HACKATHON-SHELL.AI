    ...
    for i,turb1 in enumerate(turb_coords):
        for turb2 in np.delete(turb_coords, i, axis=0):
            if  np.linalg.norm(turb1 - turb2) < 4*turb_diam:
                ...
                print(colors.RED+'Constraint Violeted between points ',turb1,turb2)
...
def greedy_F(k,dis,turb_rad,power_curve,XI_x,XI_y,dc):
    ...
    # XI_y.append(r_y-1)
    boundary_p=[]
    for i in np.arange(50.0,3951,dis):
        boundary_p.append([i,50])
        boundary_p.append([50,i])
        boundary_p.append([3950,i])
        boundary_p.append([i,3950])
    ...
    while i<k:
        ...
        flag=0
        max_power=0.000
        n_wind_instances,cos_dir,sin_dir,wind_sped_stacked,C_t = preProcessing(power_curve,i+1)
        #print("ye hhhhhhhh  ")


        #Select unchosen point p such that  objective function is maximaizied 
        
        for pt in boundary_p:
            ...
            if(s==0):
                ...
                #print(pt)
                turb_coordss=[]
                turb_coordss.append(pt)
                turb_coordss=np.array(turb_coordss,dtype=np.float)
                AEP=0.0
                for wind_inst_freq in dc:
                    AEP=AEP+getAEP(turb_rad, turb_coordss, power_curve, dc[wind_inst_freq],n_wind_instances, cos_dir, sin_dir, wind_sped_stacked,C_t,ppp)
                ...
                if(feasible(pt[0],pt[1],XI_x,XI_y)):
                    #print("ye hhhhhhhh now for dusra ")
                    rr=pt[0]
                    cc=pt[1]
                    flag=1
        ...
        if flag==1:
            ...
            print(colors.RED+"only" +str(i+1)+"points can be found")
            break


    #print('AT the end before return we have offset-',offset)
    return(XI_x,XI_y,res)
...
if __name__ == "__main__":
    ...
    collect=[]
    cho=1
    cho=int(input('Give the no of times ou want to test/default is 1  : --> :'))
    v=1
    g=[]
    g.append('')
    XI_y=[]
    ...
    while v<=cho:
        ...
        gg=input(colors.ENDC+"Enter file name you want to save for the generated boundary points :"+"for iteration : "+str(v)+": ")
        g.append(gg)
        dist=int(input(colors.RED+"Enter min distance to consider between points on boundary in above file:"+"for iteration : "+str(v)+": "))
        dis.append(dist)
        boun=int(input(colors.ENDC+"Total no of poinst you want to keep on boundary (keep less than 39):"))
        if boun>38:
            print(colors.RED+"hey i warned u as you have kept boundary points less than 39 so i m taking it as 38.")
            boun=38
        on_boundary.append(boun)
    ...
    while v<=cho:
        ...
        #print(rows,columns)
        print(colors.GREEN+"Now lets start: for boundary points...."+"iteration : "+str(v)+"\n\n")
        XI_x=[]
        XI_y=[]
        ...
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>",on_boundary[v],dis[v],g[v])
        XI_x,XI_y,res = greedy_F(on_boundary[v],dis[v],turb_rad,power_curve,XI_x,XI_y,dc)
        ...
        turb_coords=[]
        for i,j in zip(XI_x,XI_y):
            print(colors.BLUE+">",t," [",i,j,"]")
            turb_coords.append([i,j])
            t+=1
        turb_coords=np.array(turb_coords,dtype=float)
        ...
        print(colors.PURP+'Calculated coordinates of turbine now saving in file......')
    ...
    #------------------------------------------------------------ ...............---------------------------
        ...
        n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t = preProcessing(power_curve,res)
        checkConstraints(turb_coords, turb_diam)
        ...
        print(colors.ENDC+'Total power produced by the wind farm is: ', "%.12f"%(AEP), 'GWh')
        ...
        #print('\ntime-taken'+str(pin))
        print(colors.BLUE+'Actual time in min approx--<>:',s)
        print("\n\n")
        v+=1