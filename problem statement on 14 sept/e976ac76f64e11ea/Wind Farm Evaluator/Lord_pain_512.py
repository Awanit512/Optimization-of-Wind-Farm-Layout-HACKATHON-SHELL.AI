...
if __name__ == "__main__":
    ...
    wind_inst_freq1 = binWindResourceData(r'../Shell_Hackathon Dataset/Wind Data/wind_data_2008.csv')
    ...
    v=0
    while v<cho:
        g='AR_'+str(collect[v][0])+'_'+str(collect[v][1])
        d=collect[v][2]
        G_r_x,G_c_y,rows,columns =construct_G(start_x,start_y,end_x,end_y,d)
        #print("##\n")
        print(rows,columns)
        S=[]
        S=construct_S(rows,columns)
        print("Now lets start: for point"+str(v)+"\n\n")
        r_x=collect[v][0]
        r_y=collect[v][1]
        XI_x=[]
        XI_y=[]
        ...
        XI_x,XI_y = greedy_F(50,0,G_r_x,G_c_y,0,rows,0,columns,S,r_x,r_y,turb_rad,power_curve,XI_x,XI_y,dc)
