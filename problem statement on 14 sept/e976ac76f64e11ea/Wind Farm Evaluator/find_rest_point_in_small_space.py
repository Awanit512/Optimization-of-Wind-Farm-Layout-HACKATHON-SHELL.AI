...
def greedy_F(x1,x2,y1,y2,k,dis,turb_rad,power_curve,XI_x,XI_y,dc):
    print('Greedy-F')
    ...
    while i<k:
        ...
        for rr in np.arange(x1,x2+1,dis):
            for cc in np.arange(y1,y2+1,dis):
        ...
        if flag==1: 
            ...
            print('only'+str(i-k+50)+'points can be possible in your given space area x1,x2,y1,y2 respectively:'+str(x1)+" "+str(x2)+" "+str(y1)+" "+str(y2))
            print('Enjoy..........')
            break
...
if __name__ == "__main__":
    ...
    while v<=cho:
        ...
        dis=float(input("Enter min distance taken in above file:"))
        x11=float(input("Enter x1(note it should be in range [0,3950]):"))
        x22=float(input("Enter x2:(note it should be in range [0,3950]):"))
        y11=float(input("Enter y1:(note it should be in range [0,3950]):"))
        y22=float(input("Enter y2:(note it should be in range [0,3950]):"))
        x1=min(x11,x22)
        x2=max(x11,x22)
        y1=min(y11,y22)
        y2=max(y22,y11)
    ...
    while v<cho:
        ...
        XI_x,XI_y = greedy_F(x1,x2,y1,y2,k,dis,turb_rad,power_curve,XI_x,XI_y,dc)
