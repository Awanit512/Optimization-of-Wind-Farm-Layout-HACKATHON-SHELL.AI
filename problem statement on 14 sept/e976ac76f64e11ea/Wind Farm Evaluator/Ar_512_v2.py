...
def construct_S(row,col):
    print('I am in  Construct_S   #S')
    S=[]
    for i in range(row):
        coll=[]
        for j in range(col):
...
def construct_G(sx,sy,ex,ey):
    ...
    for i in np.arange(sx+0.0,ex+50.0,50.0):
        col_r=[]
        col_c=[]
        for j in np.arange(sy+0.0,ey+50.0,50):
            col_r.append(i)
            col_c.append(j)
            if flag==0:
                column+=1
...
def feasible(rr,cc,XI_x,XI_y,G_r_x,G_c_y):
   # if((rr+cc)%100==0):
...
def greedy_F(k,offset,G_r_x,G_c_y,rs_start,rows,cs_start,columns,S,r_x,r_y,turb_rad,power_curve, wind_inst_freq,XI_x,XI_y):
    print('Greedy-F')
    ...
    while i<k+offset:
        ...
        for rr in range(rs_start,rows,1):
            for cc in range(cs_start,columns,1):
                if S[rr][cc] == 1:
                    continue
                # condition 1 chosen p is feasible or not
                if(feasible(rr,cc,XI_x,XI_y,G_r_x,G_c_y)):
                    ...
                    if F(Cp,rr,cc,XI_x,XI_y,G_r_x,G_c_y) >= offset+k-i-1 :
                        ...
                        for ih,jh in zip(XI_x,XI_y):
                            turb_coords.append([G_r_x[ih][jh],G_c_y[ih][jh]])

                        turb_coords.append([ G_r_x[rr][cc],G_c_y[rr][cc] ])

                        turb_coords=np.array(turb_coords,dtype=np.float)
                        ...
                        AEP=getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq,n_wind_instances, cos_dir, sin_dir, wind_sped_stacked,C_t,ppp) 
                        if(ppp<10):
                            print(AEP)
                        ppp+=1
                        if (max_power<AEP):
    ...
    print('AT the end before retur we have offset-',offset)
    return(XI_x,XI_y)




def save(XI_x,XI_y,G_r_x,G_c_y,s):
...
if __name__ == "__main__":
    ...
    g=input('Give the name of file you want to save as: (ex: Team7_turb_coords_1)  --> :')
    ...
    wind_inst_freq =  binWindResourceData(r'../Shell_Hackathon Dataset/Wind Data/wind_data_2013.csv')   
    ...
    end_x=3950
    end_y=3950
    G_r_x,G_c_y,rows,columns =construct_G(start_x,start_y,end_x,end_y)
    #print("##\n")
    print(rows,columns)
    S=[]
    S=construct_S(rows,columns)
    ...
    # S[32][11]=1
    # S[28][4]=1
    XI_x,XI_y = greedy_F(k,0,G_r_x,G_c_y,rs_start,40,cs_start,40,S,r_x,r_y,turb_rad,power_curve, wind_inst_freq,XI_x,XI_y)
    t=1
    for i,j in zip(XI_x,XI_y):
    ...
    XI_x.append(78)
    XI_y.append(78)
    XI_x,XI_y = greedy_F(k,12,G_r_x,G_c_y,40,79,40,79,S,r_x,r_y,turb_rad,power_curve, wind_inst_freq,XI_x,XI_y)
    ...
    XI_x.append(0)
    XI_y.append(78)
    XI_x,XI_y = greedy_F(k,24,G_r_x,G_c_y,0,40,40,79,S,r_x,r_y,turb_rad,power_curve, wind_inst_freq,XI_x,XI_y)
    ...
    XI_x.append(78)
    XI_y.append(0)
    XI_x,XI_y = greedy_F(k,36,G_r_x,G_c_y,40,79,0,40,S,r_x,r_y,turb_rad,power_curve, wind_inst_freq,XI_x,XI_y)
    ...
    print('Calculated coordinates of turbine now saving......')
    

    save(XI_x,XI_y,G_r_x,G_c_y,g)
    ...
    turb_coords=tt.placed_two_turbines(79,79,turb_rad, turb_coords, power_curve, wind_inst_freq)
    ...
    AEP = getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq, n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t,1) 
    print('Total power produced by the wind farm is: ', "%.12f"%(AEP), 'GWh')
